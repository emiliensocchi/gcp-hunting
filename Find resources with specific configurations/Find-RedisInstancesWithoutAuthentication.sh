#### Description #################################################################################
#
# Indexes all instances of Cloud Memorystore for Redis in an environment which do not enforce 
# authentication.
#
####
      ...
      then
            ...
            do
                  ...
                  do
                        ...
                        gcloud beta redis instances get-auth-string "$redis_instance" --project "$project" --region "$region" 2>/dev/null

                        if [[ $? -eq 1 ]]
                        then
                              echo "[!] Authentication is disabled: $redis_instance"
